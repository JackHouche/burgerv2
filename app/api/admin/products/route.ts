...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Attempting to create product:", validatedData.name);
    ...
    try {
      ...
      if (validatedData.ingredients && validatedData.ingredients.length > 0) {
        ...
        console.log("Ingredients added:", validatedData.ingredients.length);
      }
      ...
      // Si la DB n'est pas configurée, retourner un message explicite
      if (
        dbError instanceof Error &&
        dbError.message.includes("Request context not available")
      ) {
        return NextResponse.json(
          {
            error: "Database not configured",
            message:
              "Cloudflare D1 database is not properly configured. Please initialize the database schema first.",
            instructions:
              "Run: node scripts/init-db.mjs for setup instructions",
          },
          { status: 503 },
        );
      }
